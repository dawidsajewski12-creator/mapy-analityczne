...
def rasterize_points_numba(points, min_x, max_y, res, nx, ny):
    ...
    for i in prange(points.shape[0]):
        col = int((points[i, 0] - min_x) / res); row = int((max_y - points[i, 1]) / res)
        if 0 <= row < ny and 0 <= col < nx: atomic_max(grid_max, (row, col), points[i, 2])
    for i in prange(ny):
        for j in range(nx):
            if np.isinf(grid_max[i, j]): grid_max[i, j] = np.nan
    return grid_max
@njit
def atomic_max(array, idx, value):
    if value > array[idx]: array[idx] = value
...
def main(config):
    ...
    with rasterio.open(paths['nmt']) as src_nmt:
        profile = src_nmt.profile; nmt = src_nmt.read(1)
        ny, nx = nmt.shape; minx, maxy = profile['transform'] * (0, 0); pixel_width = profile['transform'].a
    laz_files = [os.path.join(paths['laz_folder'], f) for f in os.listdir(paths['laz_folder']) if f.endswith('.laz')]
    if not laz_files: raise FileNotFoundError(f"Nie znaleziono plików .laz w folderze: {paths['laz_folder']}")
    ...
    chm = vnmpt_filled - nmt; chm[chm < 0] = 0
    ...
    with rasterio.open(paths['output_crowns_raster'], 'w', **profile) as dst: dst.write(labels.astype(rasterio.int32), 1)
    print(f"  -> Raster koron drzew zapisany.")
    tree_data = []; unique_final_labels = np.unique(labels)[1:]
    for tree_id in unique_final_labels:
        current_crown_mask = (labels == tree_id); chm_crown = np.where(current_crown_mask, chm, -np.inf)
        flat_index = np.argmax(chm_crown); treetop_row, treetop_col = np.unravel_index(flat_index, chm_crown.shape)
        height_relative = chm[treetop_row, treetop_col]; height_absolute = nmt[treetop_row, treetop_col] + height_relative
    ...
    print("--- Skrypt 0 (Lidar) zakończony ---")
    return True
