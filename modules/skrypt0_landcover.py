...
# ### POPRAWKA: Zmieniono nazwę funkcji z `main` na `run_landcover_analysis` ###
def run_landcover_analysis(config):
    """Główna funkcja modułu, tworzy raster landcover.tif."""
    print("\n--- Uruchamianie Skryptu 0: Tworzenie Pokrycia Terenu ---")
    
    paths = config['paths']
    params = config['params']

    if not os.path.exists(os.path.dirname(paths['output_landcover_raster'])):
        os.makedirs(os.path.dirname(paths['output_landcover_raster']))
